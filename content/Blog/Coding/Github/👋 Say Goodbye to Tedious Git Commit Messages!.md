...
* **auto-commit-msg** ([GitHub: MichaelCurrin/auto-commit-msg](https://github.com/MichaelCurrin/auto-commit-msg)): A VS Code extension that generates commit messages based on file changes *without* AI. It infers Conventional Commit prefixes from paths and file extensions.
...
Most of these tools cleverly analyze the *changes* (`git diff`) you've made, rather than the entire file history for each commit, ensuring messages are specific to the work being committed.
